...
from blog.posts.views import get_post
from flask import Blueprint, abort, flash, render_template, request, \
        session, url_for, redirect, g
...
def preprocess(comment, post, edit=False, delete=False):
    ...
    if not validate_token():
        if edit:
            return redirect(url_for('comments.edit', id=comment.id))

        if delete:
            return redirect(url_for('comments.delete', id=comment.id))

        return redirect(url_for('comments.create'))
